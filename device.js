	    	...
	    	if(err)
	    		...
	    		dh.openChannel(function(err,res){
                  ...
                  soapclient.on().done(function(){
                    cmd.update({"command":"dlink/switch","status":"OK","result":{"message":"Switch ON!"}},function(err,res){
										            if(err)
										                console.log(err);
										            else
                  ...
                  soapclient.off().done(function(){
                    cmd.update({"command":"dlink/switch","status":"OK","result":{"message":"Switch OFF!"}},function(err,res){
										            if(err)
										                console.log(err);
										            else
										                console.log("Command updated");
								...
								read_data();
                cmd.update({"command":"dlink/init","status":"OK","result":{"message":"Started sending data!"}},function(err,res){
										if(err)
										console.log(err);
										else
										console.log("Command updated");
...
function read_data(){
	...
	setInterval(function(){
		soapclient.consumption().done(function (power) {
				...
				var energy = power/360;
				dh.sendNotification("dlink/init",{"power":power,"energy":energy},function(err,res){
					if(err)
						console.log(err);
					else {
            console.log({"power":power,"energy":energy});
					}
				})
